...
describe("ERC721LendWrapper lends to borrower by minting an ERC-721", function () {
  ...
  beforeEach(async () => {
    ...
    lendWrapper = await LendWrapper.deploy(
      myNft.address,
      await myNft.name(),
      await myNft.symbol()
    );
  ...
  it("Should allow an ERC721 owner to lend out a token", async () => {
    await expect(
      await lendWrapper
        .connect(lender1)
        .lendOut(0, borrower1.address, startTime, 1000)
    )
  ...
  it("Should revert if the owner doesn't hold the specified token", async () => {
    await expect(
      lendWrapper
        .connect(lender1)
        .lendOut(2, borrower1.address, startTime, 1000)
    ).to.be.revertedWith("MsgSender is not owner of wrapped token.");
  });
  ...
  it("Should allow a different ERC721 owner to lend out a token previously lent out by a different owner", async () => {
    ...
    await lendWrapper
      .connect(lender1)
      .lendOut(0, borrower1.address, timeNow, 1000);
    ...
    await myNft
      .connect(lender1)
      .transferFrom(lender1.address, lender2.address, 0);
    ...
    expect(
      await lendWrapper
        .connect(lender2)
        .lendOut(0, borrower2.address, timeNow, 1000)
    )
      .to.emit(lendWrapper, "Lent")
  ...
  it("Should allow multiple lending at the same time", async () => {
    ...
    expect(
      await lendWrapper
        .connect(lender1)
        .lendOut(1, borrower2.address, startTime, 1000)
    )
      .to.emit(lendWrapper, "Lent")
    ...
    expect(
      await lendWrapper
        .connect(lender2)
        .lendOut(2, borrower2.address, startTime, 1000)
    )
      .to.emit(lendWrapper, "Lent")
...
describe("ERC721LendWrapper should return correct virtual owner", function () {
  ...
  beforeEach(async () => {
    ...
    await lendWrapper
      .connect(lender1)
      .lendOut(0, borrower1.address, startTime, 1000); // valid duration
    await myNft.connect(lender1).mint();
    await myNft.connect(lender1).approve(lendWrapper.address, 1);
    await lendWrapper
      .connect(lender1)
      .lendOut(1, borrower1.address, startTime, 0); // simulate expiry
    await myNft.connect(lender2).mint();
    await myNft.connect(lender2).approve(lendWrapper.address, 2);
    await lendWrapper
      .connect(lender2)
      .lendOut(2, borrower2.address, startTime, 1000); // valid duration
    await myNft.connect(lender2).mint();
    await myNft.connect(lender2).approve(lendWrapper.address, 3);
    await lendWrapper
      .connect(lender2)
      .lendOut(3, borrower2.address, startTime + 10000, 1000); // simulate pre-lending
  });
  ...
  it("Should allow borrower to transfer the token", async () => {
    await lendWrapper
      .connect(borrower1)
      .transferFrom(borrower1.address, borrower2.address, 0);
  ...
  it("Should not allow lender to collect token if lending period has not expired", async () => {
    await expect(lendWrapper.connect(lender1).collect(0)).to.be.revertedWith(
      "Lend is still active"
    );
    await expect(lendWrapper.connect(lender2).collect(2)).to.be.revertedWith(
      "Lend is still active"
    );
    await expect(lendWrapper.connect(lender2).collect(3)).to.be.revertedWith(
      "Pending start of lend period"
    );
  });

  async function assertBorrowerCanSurrender(
    tokenId: number,
    lender: SignerWithAddress,
    borrower: SignerWithAddress
  ) {
    const initialBalance = (
      await lendWrapper.balanceOf(borrower.address)
    ).toNumber();
    ...
    await expect(lendWrapper.ownerOf(tokenId)).to.be.revertedWith(
      "ERC721: owner query for nonexistent token"
    ); // TODO: Null address
    await expect(lendWrapper.virtualOwnerOf(tokenId)).to.be.revertedWith(
    ...
    expect(await lendWrapper.balanceOf(borrower.address)).to.equal(
      initialBalance - 1
    );
  }
